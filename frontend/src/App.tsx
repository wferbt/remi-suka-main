    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-[#121417] text-white' : 'bg-[#F4F7F9] text-[#2D3436]'}`}>
            ...
            {CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className="flex-shrink-0 flex flex-col items-center">
                <div className={`w-20 h-20 rounded-[28px] flex items-center justify-center mb-2 transition-all duration-300 border-2 overflow-hidden ${
                  selectedCategory === cat.id ? 'border-[#E63946] bg-white scale-110 shadow-xl' : `${isDark ? 'bg-[#1a1d21] border-transparent' : 'bg-white border-transparent'}`
                }`}>
                  <img src={cat.img} alt={cat.name} className="w-full h-full object-cover" />
                </div>
                {/* ТЕКСТ КАТЕГОРИИ */}
                <span className={`text-[11px] font-black uppercase tracking-widest ${selectedCategory === cat.id ? 'text-[#E63946]' : 'opacity-60'}`}>
                  {cat.name}
                </span>
              </button>
            ))}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {products.filter(p => p.name.toLowerCase().includes(selectedCategory.toLowerCase())).map(product => {
                  ...
                  return (
                    <div key={product.externalId} className={`group rounded-[35px] overflow-hidden transition-all duration-500 ${isDark ? 'bg-[#1a1d21]' : 'bg-white shadow-xl shadow-black/5 hover:shadow-2xl'}`}>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {!inCart ? (
                            ...
                            <div className="flex items-center gap-2 bg-gray-500/10 rounded-2xl p-1.5 backdrop-blur-sm">
                              <button onClick={() => updateQuantity(product, -1)} className="w-8 h-8 flex items-center justify-center bg-white dark:bg-white/10 rounded-xl shadow-sm text-[#E63946]"><Minus size={18} strokeWidth={3}/></button>
                              <span className="font-black w-6 text-center text-sm">{inCart.quantity}</span>
                              <button onClick={() => updateQuantity(product, 1)} className="w-8 h-8 flex items-center justify-center bg-[#E63946] text-white rounded-xl shadow-sm"><Plus size={18} strokeWidth={3}/></button>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          <div className="lg:col-span-1">
            <div className={`p-6 rounded-[40px] sticky top-24 border ${isDark ? 'bg-[#1a1d21] border-white/5' : 'bg-white shadow-2xl border-transparent'}`}>
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-black italic">ЗАКАЗ</h2>
              ...
              {paymentStep === 'none' ? (
                <>
                  <div className="space-y-4 mb-8 max-h-[40vh] overflow-y-auto pr-2 scrollbar-hide">
                    {cart.map(item => (
                      <div key={item.externalId} className="flex flex-col gap-3 bg-gray-500/5 p-4 rounded-[24px] border border-transparent hover:border-[#E63946]/20 transition-all">
                        <div className="min-w-0">
                          <p className="font-bold text-[11px] truncate uppercase opacity-60 mb-1">{item.name}</p>
                          <p className="font-black text-sm text-[#E63946]">{item.price * item.quantity} ₸</p>
                        </div>
                        <div className="flex items-center justify-between mt-1">
                            <span className="text-[10px] font-bold opacity-30 italic">{item.quantity} шт</span>
                            <div className="flex items-center gap-2">
                                <button onClick={() => updateQuantity(item, -1)} className="w-7 h-7 flex items-center justify-center bg-white dark:bg-white/10 rounded-lg shadow-sm text-[#E63946] active:scale-90 transition-all"><Minus size={14} strokeWidth={3}/></button>
                                <button onClick={() => updateQuantity(item, 1)} className="w-7 h-7 flex items-center justify-center bg-[#E63946] text-white rounded-lg shadow-sm active:scale-90 transition-all"><Plus size={14} strokeWidth={3}/></button>
                            </div>
                        </div>
                      </div>
                    ))}
                    {cart.length === 0 && <p className="text-center py-10 font-bold opacity-20 uppercase text-xs tracking-widest">Пусто</p>}
                  </div>
                  <div className="border-t border-dashed border-gray-500/20 pt-6">
                    <div className="flex justify-between items-center mb-6">
                      <span className="font-bold opacity-40 uppercase text-[10px] tracking-widest text-center">Итого к оплате</span>
                ...
                <div className="py-20 text-center animate-in zoom-in duration-500">
                  ...
                  <p className="text-xs font-bold opacity-40 uppercase">Заказ принят в работу</p>
                </div>
              )}
            </div>
          </div>
      ...
      {showLogin && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl bg-black/40">
          <div className={`w-full max-w-md p-10 rounded-[45px] shadow-2xl ${isDark ? 'bg-[#1a1d21]' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-10">
              <h2 className="text-4xl font-black tracking-tighter">Вход</h2>
              <button onClick={() => setShowLogin(false)} className="p-2 bg-gray-500/10 rounded-full hover:rotate-90 transition-transform"><X size={20}/></button>
            </div>
            <form onSubmit={handleLogin} className="space-y-5">
              <input required name="username" placeholder="Логин" className="w-full p-5 rounded-[22px] bg-gray-500/5 outline-none border-2 border-transparent focus:border-[#E63946] font-bold" />
              <input required name="password" type="password" placeholder="Пароль" className="w-full p-5 rounded-[22px] bg-gray-500/5 outline-none border-2 border-transparent focus:border-[#E63946] font-bold" />
              <button type="submit" className="w-full py-5 bg-[#E63946] text-white rounded-[22px] font-black text-lg shadow-2xl shadow-red-500/30">ВОЙТИ</button>
            </form>
          </div>
        </div>
      )}
